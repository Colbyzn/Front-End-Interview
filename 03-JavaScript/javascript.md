...
2. **按需导出与导入**
   ...
   - 语法格式
     ...
     // 按需导入
     import { 接受名称1, 接受名称2, ... } from '所要导入的模块文件地址';
     ```
     ...
     > 3. 按需导入中若想更给接收名称，通过格式「**接收名称 as 新名称**」来更改
     > 4. 上述按需导入的 import 语句并**不是解构赋值，而是命名导入**，与解构赋值的区别：import 语句中接收的名称必须是所导入模块中**定义的变量名，不能是对象的属性名/方法名**，而解构赋值可以直接解构出对象的属性名/方法名
