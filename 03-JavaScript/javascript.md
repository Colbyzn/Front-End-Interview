...
1. **使用 `Boolean()` 函数**
   ...
   - 语法
     ...
     > 注：代表**空、否定的值**（比如：**''、0、NaN、null、undefined**）会被**转换为 false**，其余值都会被转换为 true
...
| `unshift` | 在数组的**开头添加**一个或多个元素 | **新数组的长度**                               | 是             |
...
- 以下是一个纯函数的例子，它没有修改任何外部数据，且输出完全由输入决定，输出的数据类型可以与输入的数据类型不同
...
- 以下是一个非纯函数的例子，虽然输出完全由输入决定，但是它修改外部数据 result，即有副作用
...
- `every()`
- `reduce()`
  > 注：若 some()、every()、reduce() 所传入的回调函数是纯函数，那么它们也是纯函数，否则，就不是
...
- `sort()`
- `splice()`
  > 注：以上方法虽然始终返回一个完全由传入参数决定的输出，但是它们都修改原数组
- `forEach()`
  > 注：尽管 forEach() 本身不修改原数组，但是它没有返回任何值，即输出不完全由输入决定
